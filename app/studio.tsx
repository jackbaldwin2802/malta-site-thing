  ...
  const handleCreate = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (modal === "upload") {
        ...
        if (!files.length) throw new Error("Choose one or more images or videos.");
        const caption = String(values.caption || "");
        const pending = files.map((file) => ({
          file,
          temporaryId: `uploading-${crypto.randomUUID()}`,
          localUrl: URL.createObjectURL(file),
        }));
        setMedia((items) => [...pending.map(({ file, temporaryId, localUrl }) => ({ id: temporaryId, filename: file.name, caption, status: "Draft", mimeType: file.type, uploadedBy: "Malta team", usedCount: 0, tone: "crimson", url: localUrl })), ...items]);
        setModal(null); form.reset(); setNotice(`Uploading ${files.length} item${files.length === 1 ? "" : "s"}…`);
        const results = await Promise.allSettled(pending.map(async ({ file, temporaryId, localUrl }) => {
          const saved = await uploadMediaFile(file, caption);
          setMedia((items) => items.map((item) => item.id === temporaryId ? { ...item, id: String(saved.id), url: String(saved.url || localUrl) } : item));
          URL.revokeObjectURL(localUrl);
        }));
        const failedIds = pending.filter((_, index) => results[index].status === "rejected").map((item) => item.temporaryId);
        if (failedIds.length) {
          setMedia((items) => items.filter((item) => !failedIds.includes(item.id)));
          pending.filter((item) => failedIds.includes(item.temporaryId)).forEach((item) => URL.revokeObjectURL(item.localUrl));
        }
        const uploaded = files.length - failedIds.length;
        setNotice(failedIds.length ? `${uploaded} uploaded · ${failedIds.length} failed` : `${uploaded} item${uploaded === 1 ? "" : "s"} uploaded`);
      } else if (modal === "post") {
  ...
  return (
    <SidebarProvider defaultOpen>
      <Sidebar collapsible="icon" className="border-r border-white/10 bg-[#071d2b] text-white">
        <SidebarHeader className="p-4"><div className="flex items-center gap-3 px-1 py-2"><img src="/malta-flag-logo.png" alt="Malta flag" className="h-10 w-12 shrink-0 object-contain" /><div className="group-data-[collapsible=icon]:hidden"><p className="text-lg font-bold tracking-[-0.04em]">MALTA</p><p className="text-xs tracking-[0.08em] text-white/45">Media Management</p></div></div></SidebarHeader>
